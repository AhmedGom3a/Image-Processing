...
for table in tables:
    ...
    for i in range(len(table_entries)):
        ...
        for j in range(len(row)):
            entry = row[j]
            entry_roi = table_roi[(entry[1] + border_size) * mult: (entry[1] + entry[3]) * mult, (entry[0] + border_size) * mult:(entry[0] + entry[2]) * mult]

            struct = cv.getStructuringElement(cv.MORPH_ELLIPSE, (3, 3))
            entry_roi = cv.dilate(entry_roi, struct, iterations = 1)
